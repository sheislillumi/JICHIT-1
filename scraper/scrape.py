...
KEYWORDS_TOPIC = [
    "展示", "商談", "見本市", "物産展", "産業展",
    "販路拡大", "販路開拓", "流通拡大", "販売促進", "市場開拓",
    "マッチング", "バイヤー", "求評会",
    "DX", "デジタル化", "脱炭素", "カーボンニュートラル", "GX",
]
...
# など無関係分野を拾ってしまう(福岡市の実例で確認)。「DX」「脱炭素」等も
# 同様に単体では一般のDX推進事業・脱炭素検討業務まで拾ってしまうため汎用語
# として扱う。他により具体的なトピックキーワードが一致していない場合に限り、
# これらの語が併記されていたら誤検知とみなして除外する。
GENERIC_TOPIC_KEYWORDS = {"マッチング", "バイヤー", "DX", "デジタル化", "脱炭素", "カーボンニュートラル", "GX"}
NEGATIVE_KEYWORDS = [
    "ふるさと納税", "人材", "雇用", "子ども食堂", "介護", "システム開発", "セキュリティ", "算定",
    # DX/脱炭素/GX/デジタル化/カーボンニュートラルを汎用語として追加した際、
    # 展示会・商談会と無関係な職員研修・セミナー・経営支援コンサル・政策系業務が
    # 大量に誤検知したため(令和8年度分の初回実行で新規ヒット81件中75件が誤検知)、
    # それらに頻出する語を追加(実データで確認済み)。
    "研修", "セミナー", "講習", "フォーラム", "ハイスクール", "太陽光発電",
    "理解促進", "先行地域", "育成", "水素社会", "森林資源", "水利施設", "経営",
    "コーディネート", "ビジネスモデル創出", "チャレンジサポート", "予約システム",
    "砂防", "クレジット", "ナレーション", "行政手続", "伴走支援", "農村整備",
    "生成AI", "経理",
    # 1回目の負ワード追加後もなお一致していた案件から追加(実データで確認済み)。
    "行動変容", "補助金", "自家消費", "成長支援", "働き方", "地域脱炭素化",
    "地域貢献", "統合イベント", "アクション創出", "産業集積", "事例集",
    "ビジネススタイル転換", "活用方策", "許可証",
]
